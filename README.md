...
## A second review found the community-feedback fixes were only partial — closed out properly this round

A prior round addressed a playtester's (Terry Lee, via the "Game Mission" community post) feedback but a direct code review found three of the four points were incomplete, not fully done as the summary had implied:

- **Keyboard controls in the tutorial**: a persistent "Keyboard: keys 1–4" hint was added to the always-visible control panel, but the actual tutorial dialog (step 4) still only ever said `press the "1" key` — no line in "How to Play" itself mentioned keys 2–4 existed. Fixed by rewording step 4 to `press the "1" key — all four abilities below have their own key, 1 to 4`, in both languages.
- **`return_to_menu` tracking**: only wired to the in-combat Home button (`btn-home`). The Victory, Defeat, and Grid Overdrive end screens each have their own "Return to Base" button, and the Upgrades screen has its own "Back" button — none of the four called `trackEvent`. Fixed by adding the same `trackEvent('return_to_menu')` call to `btn-win-home`, `btn-lose-home`, `btn-timed-home`, and `btn-upgrades-back`, in both files. Verified live: a real campaign victory followed by clicking "Return to Base" on the victory screen now produces `["tutorial_complete","first_incident_cleared","return_to_menu"]` in `localStorage`'s `gb_events_v1`.
- **Touch handling**: the game only ever used `click` listeners with no touch-specific CSS, and real touch hardware still cannot be tested from this environment — that limitation is unchanged and stays documented below. What *was* fixed: `touch-action:manipulation` is now set on `body` and on every interactive element (`.cell`, `.ability`, `.btn`, `.icon-btn`), which removes the ~300ms tap-delay and disables double-tap-to-zoom that mobile browsers apply by default to a plain `click`-based UI — a real, verifiable improvement to tap responsiveness, confirmed via `getComputedStyle(...).touchAction === 'manipulation'` on both `body` and a board cell. This does not substitute for testing on a physical touchscreen, which remains an open gap.
- **Language persistence** was the one point already fully correct from the prior round — re-confirmed, no change needed.
